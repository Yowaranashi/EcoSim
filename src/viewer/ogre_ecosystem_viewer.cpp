...
struct OgreEcosystemViewer::Impl {
    ...
    void createFont() {
        auto font = Ogre::FontManager::getSingleton().getByName(font_name);
        if (font.isNull()) {
            font = Ogre::FontManager::getSingleton().create(font_name, Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME);
            font->setType(Ogre::FT_TRUETYPE);
#if defined(_WIN32)
            Ogre::ResourceGroupManager::getSingleton().addResourceLocation(
                "C:/Windows/Fonts", "FileSystem", Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME);
